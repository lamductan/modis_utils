...
from tensorflow.python.keras.layers import Convolution1D, MaxPooling1D, Conv2DTranspose
...
def _create_model_with_tensorflow_2(model_params, compile_params):
    ...
    reconstruct_imgs = TimeDistributed(Conv2D(filters=1,
                               kernel_size=kernel_size_tuple,
                               strides=strides,
                               activation=output_activation,
                               padding=padding, 
    ...
    predict_layers[0] = ConvLSTM2D(filters=filters[0], 
                                    kernel_size=kernel_size_tuple,
                                    strides=strides,
                                    padding=padding,
                                    data_format=data_format,
    ...
    for i in range(1, n_hidden_layers - 1):
        predict_layers[i] = ConvLSTM2D(filters=filters[i], 
                                        kernel_size=kernel_size_tuple,
                                        strides=strides,
                                        padding=padding,
                                        data_format=data_format,
    ...
    predict_layers[-1] = ConvLSTM2D(filters=filters[-1], 
                                    kernel_size=kernel_size_tuple,
                                    strides=strides,
                                    padding=padding,
                                    data_format=data_format,
...
# Custom model
def conv_lstm_2D(filters, kernel_size, strides, padding='same', 
...
def conv_2D(filters, kernel_size, strides, padding='valid', activation='tanh', dilation_rate=(1,1)):
    return Conv2D(filters=filters, kernel_size=kernel_size, 
                  strides=strides, padding=padding,
                  activation=activation, dilation_rate=dilation_rate)

def conv_2D_transpose(filters, kernel_size, strides, padding='valid', activation='tanh', dilation_rate=(1,1)):
